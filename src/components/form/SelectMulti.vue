  ...
  >
    ...
    <div class="select-box" @click="visible = !visible">
      ...
      <ul class="select-box-item" v-if="visible">
        <li
          :class="['ellipsis', { selected: isSelected(all) }]"
          v-if="options.length > 1 && allTitle"
          @click="onChange(all)"
        >
          {{ allTitle }}
        </li>
    ...
    <div class="tag-list">
      <ul v-if="selected.length !== options.length">
        <li v-for="(selectedItem, index) in selected" :key="index">
          <span class="tag-content">
            <span :class="['tag-title', { disabled: disabled }]">{{
      ...
      <ul v-else-if="selected.length > 0">
        <li>
          <span class="tag-content">
            <span
              :class="['tag-title', { disabled: disabled }]"
              v-if="allTitle"
              >{{ allTitle }}</span
            >
            <span
    ...
    isSelected(option) {
      ...
      return (
        this.selected.findIndex(
          (x) => x.toString() === option.value.toString()
        ) !== -1
      );
    },
    ...
    getTitle(selectedValue) {
      if (this.options.length > 0) {
        return this.options.find(
          (x) => x.value.toString() === selectedValue.toString()
        ).title;
      }
    },
